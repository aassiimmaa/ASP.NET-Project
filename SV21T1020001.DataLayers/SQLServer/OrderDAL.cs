...
namespace SV21T1020001.DataLayers.SQLServer
{
    public class OrderDAL : BaseDAL, IOrderDAL
    {
        ...
        public List<Order> GetOrderByUserId(int id)
        {
            ...
            {
                var sql = @"SELECT 
                        Orders.OrderID, Orders.CustomerID, OrderStatus.Status,CustomerID,OrderTime,AcceptTime,ShippedTime,FinishedTime,
                        SUM(OrderDetails.SalePrice * OrderDetails.Quantity) AS TotalAmount,
                        OrderStatus.Status
                    FROM Orders
                    JOIN OrderDetails ON Orders.OrderID = OrderDetails.OrderID
                    JOIN Products ON Products.ProductID = OrderDetails.ProductID
                    JOIN OrderStatus ON Orders.Status = OrderStatus.Status
                    WHERE Orders.CustomerID = @id
                    GROUP BY Orders.OrderID, Orders.CustomerID, OrderStatus.Status,CustomerID,OrderTime,AcceptTime,ShippedTime,FinishedTime";
                var parameters = new { id = id };
                return connection.Query<Order>(sql: sql, param: parameters, commandType: System.Data.CommandType.Text).ToList();
            }
        }
